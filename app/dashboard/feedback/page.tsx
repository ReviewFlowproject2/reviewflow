...
const StatusBadge = ({ status }: { status: string }) => {
  const styles: Record<string, string> = {
    new: "bg-blue-500/10 text-blue-400 border-blue-200",
    read: "bg-slate-950 text-slate-300 border-gray-200",
    replied: "bg-purple-50 text-purple-700 border-purple-200",
    resolved: "bg-emerald-500/10 text-emerald-400 border-green-200",
  };
...
const SentimentBadge = ({ sentiment }: { sentiment: string }) => {
  const styles: Record<string, string> = {
    ...
    neutral: "bg-amber-500/10 text-yellow-700",
  ...
  return (
    <div className="min-h-screen bg-slate-950 p-6">
      <div className="max-w-7xl mx-auto">
        <div className="mb-6">
          <Link href="/dashboard" className="inline-flex items-center gap-2 text-sm text-slate-400 hover:text-slate-300 transition-colors">
        ...
        <div className="mb-8 flex items-center justify-between">
          <div>
            ...
            <p className="text-slate-400">Manage and respond to patient reviews across all platforms.</p>
          </div>
          <div className="flex items-center gap-3">
            {syncMsg && <span className="text-xs text-emerald-400 bg-emerald-500/10 px-3 py-1.5 rounded-full">{syncMsg}</span>}
            <button onClick={fetchFromGoogle} disabled={syncing}
              className="inline-flex items-center gap-2 px-4 py-2 bg-slate-800 border-2 border-brand-blue text-emerald-400 font-semibold rounded-xl text-sm hover:bg-emerald-500 hover:text-white transition-colors disabled:opacity-50">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
          <div className="bg-slate-800 p-4 rounded-xl border border-gray-200 shadow-sm">
            <div className="flex items-center justify-between">
              <div><p className="text-sm text-slate-400">Total Reviews</p><p className="text-2xl font-bold text-white">{stats.total}</p></div>
              <MessageSquare className="text-blue-500" size={24} />
            </div>
          </div>
          <div className="bg-slate-800 p-4 rounded-xl border border-gray-200 shadow-sm">
            <div className="flex items-center justify-between">
              <div><p className="text-sm text-slate-400">New Reviews</p><p className="text-2xl font-bold text-blue-400">{stats.new}</p></div>
              <Mail className="text-blue-500" size={24} />
            </div>
          </div>
          <div className="bg-slate-800 p-4 rounded-xl border border-gray-200 shadow-sm">
            <div className="flex items-center justify-between">
              <div><p className="text-sm text-slate-400">Needs Attention</p><p className="text-2xl font-bold text-red-400">{stats.negative}</p></div>
              <Star className="text-red-500 fill-red-500" size={24} />
            </div>
          </div>
          <div className="bg-slate-800 p-4 rounded-xl border border-gray-200 shadow-sm">
            <div className="flex items-center justify-between">
              <div><p className="text-sm text-slate-400">Resolved</p><p className="text-2xl font-bold text-emerald-400">{stats.resolved}</p></div>
        ...
        {/* Filters */}
        <div className="bg-slate-800 p-4 rounded-xl border border-gray-200 shadow-sm mb-6">
          <div className="flex flex-col md:flex-row gap-4">
            ...
            <div className="flex gap-3">
              <div className="relative">
                <select className="appearance-none bg-slate-950 border border-gray-300 text-slate-300 py-2 pl-4 pr-10 rounded-lg outline-none cursor-pointer" value={statusFilter} onChange={(e) => setStatusFilter(e.target.value)}>
              ...
              <div className="relative">
                <select className="appearance-none bg-slate-950 border border-gray-300 text-slate-300 py-2 pl-4 pr-10 rounded-lg outline-none cursor-pointer" value={ratingFilter} onChange={(e) => setRatingFilter(e.target.value)}>
        ...
        <div className="space-y-3">
          {filteredReviews.length === 0 ? (
            <div className="bg-slate-800 rounded-xl border border-gray-200 p-12 text-center">
              ...
              <p className="text-slate-400 mb-4">Click "Fetch from Google" to pull real reviews from your Google Business Profile.</p>
              <button onClick={fetchFromGoogle} disabled={syncing}
                className="px-4 py-2 bg-emerald-500 text-white text-sm font-semibold rounded-lg hover:bg-emerald-600 disabled:opacity-50 inline-flex items-center gap-2">
                {syncing ? "Fetching..." : "Fetch from Google"}
              </button>
            </div>
          ) : (
            filteredReviews.map((review) => (
              <div key={review.id} className={`bg-slate-800 rounded-xl border transition-all duration-200 overflow-hidden ${review.status === "new" ? "border-blue-300 shadow-md" : "border-gray-200 shadow-sm"}`}>
                <div className="p-4 cursor-pointer hover:bg-slate-950 transition-colors" onClick={() => setExpandedId(expandedId === review.id ? null : review.id)}>
                  <div className="flex items-start justify-between gap-4">
                    <div className="flex items-start gap-4 flex-1">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-3 mb-1 flex-wrap">
                          <h3 className="font-semibold text-white">{review.patient_name}</h3>
                          <StatusBadge status={review.status} />
                          <SentimentBadge sentiment={review.sentiment} />
                          <span className="text-xs text-slate-400 flex items-center gap-1"><Clock size={12} />{formatDate(review.created_at)}</span>
                        </div>
                    ...
                    <div className="flex items-center gap-2 shrink-0">
                      {review.status === "new" && (
                        <button onClick={(e) => { e.stopPropagation(); updateStatus(review.id, "read"); }} className="p-2 text-blue-400 hover:bg-blue-500/10 rounded-lg transition-colors" title="Mark as read"><MailOpen size={18} /></button>
                      )}
                ...
                {expandedId === review.id && (
                  <div className="border-t border-slate-600 p-4 bg-slate-950/50">
                    <div className="ml-14">
                      <div className="bg-slate-800 p-4 rounded-lg border border-gray-200 mb-4"><p className="text-gray-800 leading-relaxed">{review.comment}</p></div>
                      <div className="flex flex-wrap items-center gap-3 mb-4">
                        ...
                        <button onClick={() => updateStatus(review.id, "read")} className="inline-flex items-center gap-2 px-4 py-2 bg-slate-800 border border-gray-300 text-slate-300 rounded-lg hover:bg-slate-950 transition-colors text-sm font-medium"><MailOpen size={16} />Mark as Read</button>
                        {googleLink && (
                          <a href={googleLink} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 px-4 py-2 bg-emerald-500 text-white rounded-lg hover:bg-emerald-600 transition-colors text-sm font-medium"><ExternalLink size={16} />Reply on Google</a>
                        )}
                      </div>

                      {showReplyForm === review.id && (
                        <div className="bg-slate-800 p-4 rounded-lg border border-gray-200 mb-4">
                          <div className="flex items-center justify-between mb-3">
                            <h4 className="font-medium text-white">Draft Reply</h4>
                            <button onClick={() => setShowReplyForm(null)} className="text-gray-400 hover:text-slate-400"><X size={16} /></button>
                          </div>
                          <div className="flex flex-wrap gap-2 mb-3">
                            {quickTemplates.map((template, idx) => (
                              <button key={idx} onClick={() => setReplyDraft((prev) => ({ ...prev, [review.id]: template }))} className="px-3 py-1.5 bg-gray-100 text-slate-300 rounded-md text-xs hover:bg-gray-200 transition-colors text-left">Template {idx + 1}</button>
                            ))}
                          </div>
                          ...
                          <div className="flex justify-between items-center mt-3">
                            <p className="text-xs text-slate-400">This will be saved as a draft. Direct platform reply coming soon.</p>
                            <div className="flex gap-2">
                              <button onClick={() => setReplyDraft((prev) => ({ ...prev, [review.id]: "" }))} className="px-3 py-1.5 text-sm text-slate-400 hover:bg-gray-100 rounded-lg transition-colors">Clear</button>
