      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-8 flex items-center justify-between">
          <div>
            ...
            <p className="text-gray-500">Manage and respond to patient reviews across all platforms.</p>
          </div>
          <div className="flex items-center gap-3">
            {syncMsg && <span className="text-xs text-teal-600 bg-teal-50 px-3 py-1.5 rounded-full">{syncMsg}</span>}
            <button onClick={fetchFromGoogle} disabled={syncing}
              className="inline-flex items-center gap-2 px-4 py-2 bg-white border-2 border-teal-600 text-teal-600 font-semibold rounded-xl text-sm hover:bg-teal-600 hover:text-white transition-colors disabled:opacity-50">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
          {[
            ...
            { label: "New Reviews", value: stats.new, icon: Mail, color: "text-blue-600", bg: "bg-blue-50" },
            { label: "Needs Attention", value: stats.negative, icon: Star, color: "text-red-600", bg: "bg-red-50", iconFill: true },
            { label: "Resolved", value: stats.resolved, icon: CheckCircle, color: "text-teal-600", bg: "bg-teal-50" },
          ].map((s, i) => (
            <div key={i} className="bg-white p-4 rounded-xl border border-gray-200 shadow-sm">
              <div className="flex items-center justify-between">
                ...
                <s.icon className={s.color} size={24} fill={s.iconFill ? "currentColor" : "none"} />
              </div>
            </div>
          ))}
        </div>
        ...
        <div className="bg-white p-4 rounded-xl border border-gray-200 shadow-sm mb-6">
          <div className="flex flex-col md:flex-row gap-4">
            <div className="flex-1 relative">
              ...
              <input type="text" placeholder="Search by patient name or keyword..."
                className="w-full pl-10 pr-4 py-2 border border-gray-300 rounded-lg text-gray-900 focus:ring-2 focus:ring-teal-500/30 focus:border-teal-500 outline-none"
                value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} />
            </div>
            <div className="flex gap-3">
              <div className="relative">
                <select className="appearance-none bg-white border border-gray-300 text-gray-700 py-2 pl-4 pr-10 rounded-lg outline-none cursor-pointer" value={statusFilter} onChange={(e) => setStatusFilter(e.target.value)}>
              ...
              <div className="relative">
                <select className="appearance-none bg-white border border-gray-300 text-gray-700 py-2 pl-4 pr-10 rounded-lg outline-none cursor-pointer" value={ratingFilter} onChange={(e) => setRatingFilter(e.target.value)}>
        ...
        <div className="space-y-3">
          {filteredReviews.length === 0 ? (
            <div className="bg-white rounded-xl border border-gray-200 p-12 text-center">
              ...
              <button onClick={fetchFromGoogle} disabled={syncing}
                className="px-4 py-2 bg-teal-600 text-white text-sm font-semibold rounded-lg hover:bg-teal-700 disabled:opacity-50 inline-flex items-center gap-2">
                {syncing ? "Fetching..." : "Fetch from Google"}
              </button>
            </div>
          ) : (
            filteredReviews.map((review) => (
              <div key={review.id} className={`bg-white rounded-xl border transition-all duration-200 overflow-hidden ${review.status === "new" ? "border-blue-300 shadow-sm" : "border-gray-200"}`}>
                <div className="p-4 cursor-pointer hover:bg-gray-50 transition-colors" onClick={() => setExpandedId(expandedId === review.id ? null : review.id)}>
                  <div className="flex items-start justify-between gap-4">
                    <div className="flex items-start gap-4 flex-1">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-3 mb-1 flex-wrap">
                          ...
                          <span className="text-xs text-gray-400 flex items-center gap-1"><Clock size={12} />{formatDate(review.created_at)}</span>
                        </div>
                    ...
                    <div className="flex items-center gap-2 shrink-0">
                      {review.status === "new" && (
                        <button onClick={(e) => { e.stopPropagation(); updateStatus(review.id, "read"); }} className="p-2 text-blue-500 hover:bg-blue-50 rounded-lg transition-colors" title="Mark as read"><MailOpen size={18} /></button>
                      )}
                ...
                {expandedId === review.id && (
                  <div className="border-t border-gray-200 p-4 bg-gray-50">
                    <div className="ml-14">
                      <div className="bg-white p-4 rounded-lg border border-gray-200 mb-4"><p className="text-gray-700 leading-relaxed">{review.comment}</p></div>
                      <div className="flex flex-wrap items-center gap-3 mb-4">
                        ...
                        {review.status !== "replied" && (
                          <button onClick={() => setShowReplyForm(showReplyForm === review.id ? null : review.id)} className="inline-flex items-center gap-2 px-4 py-2 bg-teal-600 text-white rounded-lg hover:bg-teal-700 transition-colors text-sm font-medium"><Reply size={16} />{showReplyForm === review.id ? "Cancel Reply" : "Draft Reply"}</button>
                        )}
                        <button onClick={() => updateStatus(review.id, "read")} className="inline-flex items-center gap-2 px-4 py-2 bg-white border border-gray-300 text-gray-600 rounded-lg hover:bg-gray-50 transition-colors text-sm font-medium"><MailOpen size={16} />Mark as Read</button>
                        {googleLink && (
                          <a href={googleLink} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 px-4 py-2 bg-teal-600 text-white rounded-lg hover:bg-teal-700 transition-colors text-sm font-medium"><ExternalLink size={16} />Reply on Google</a>
                        )}
                      </div>
                      ...
                      {showReplyForm === review.id && (
                        <div className="bg-white p-4 rounded-lg border border-gray-200 mb-4">
                          ...
                          <textarea className="w-full p-3 border border-gray-300 rounded-lg text-gray-900 focus:ring-2 focus:ring-teal-500/30 focus:border-teal-500 outline-none resize-none text-sm" rows={4} placeholder="Write your response here..." value={replyDraft[review.id] || ""} onChange={(e) => setReplyDraft((prev) => ({ ...prev, [review.id]: e.target.value }))} />
                          <div className="flex justify-between items-center mt-3">
                            <p className="text-xs text-gray-500">This will be saved as a draft. Direct platform reply coming soon.</p>
                            <div className="flex gap-2">
                              <button onClick={() => setReplyDraft((prev) => ({ ...prev, [review.id]: "" }))} className="px-3 py-1.5 text-sm text-gray-500 hover:bg-gray-100 rounded-lg transition-colors">Clear</button>
                              <button onClick={() => handleReply(review.id)} className="px-4 py-1.5 bg-teal-600 text-white rounded-lg hover:bg-teal-700 transition-colors text-sm font-medium">Save Draft</button>
                            </div>
                          </div>
                        </div>
                      )}
