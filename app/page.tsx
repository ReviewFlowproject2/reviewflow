...
function HeroSection() {
  ...
  return (
    <div ref={heroWrapperRef} className="hero-wrapper relative w-full h-screen overflow-hidden bg-brand-blue">
      ...
      <div className="hero-main-media fixed inset-0 w-screen h-screen bg-cover bg-center z-[1]" style={{
        backgroundImage: "url('/assets/clinic-interior.jpg')",
        maskImage: "radial-gradient(ellipse at center, black 30%, transparent 70%)",
        WebkitMaskImage: "radial-gradient(ellipse at center, black 30%, transparent 70%)",
        maskPosition: "center center", WebkitMaskPosition: "center center",
        maskSize: "0% 0%", WebkitMaskSize: "0% 0%",
...
function HowItWorks() {
  const steps = [
    { icon: <QrCode className="w-6 h-6" />, title: "Place QR Code", desc: "Put your custom QR code at the front desk. Patients scan it with their phone and leave a Google review instantly." },
    { icon: <Mail className="w-6 h-6" />, title: "Email Follow-Up", desc: "For patients who didn't scan the code, our system sends a polite email reminder after 24 hours." },
    { icon: <Bell className="w-6 h-6" />, title: "Monitor & Alert", desc: "New reviews appear in your dashboard. Negative reviews trigger instant email alerts with reply suggestions." },
  ];
  return (
    <section className="py-20 md:py-28 bg-white">
      <div className="max-w-6xl mx-auto px-6">
        <h2 className="font-outfit font-bold text-3xl md:text-4xl text-brand-blue text-center mb-16">How It Works</h2>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {steps.map((s, i) => (
            <div key={i} className="text-center">
              <div className="w-14 h-14 rounded-2xl bg-brand-soft flex items-center justify-center text-brand-blue mx-auto mb-5">{s.icon}</div>
    ...
    <section className="py-20 md:py-28 bg-white">
      <div className="max-w-6xl mx-auto px-6">
        <div className="text-center mb-12">
          <h2 className="font-outfit font-bold text-3xl md:text-4xl text-brand-blue mb-3">Trusted by Dental Offices Worldwide</h2>
          <p className="text-brand-muted max-w-xl mx-auto">See how practices like yours are growing their online reputation with ReviewFlow.</p>
        </div>
            ...
            <div key={i} className="bg-brand-soft rounded-[16px] p-6 border border-[#E0E7F1]">
              ...
              <div className="flex gap-1 mb-3">
                {Array.from({ length: 5 }).map((_, j) => (
                  <Star key={j} size={14} className={j < t.rating ? "fill-brand-yellow text-brand-yellow" : "text-gray-200"} />
                ))}
              </div>
              <p className="text-brand-dark text-sm leading-relaxed mb-4">{t.quote}</p>
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 rounded-full bg-brand-blue/10 flex items-center justify-center text-brand-blue font-semibold text-sm">{t.name.split(" ").map((n) => n[0]).join("")}</div>
                <div>
      ...
      <div className="max-w-6xl mx-auto px-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 items-start">
          ...
          <div className="bg-white rounded-[16px] p-8 border border-[#E0E7F1]">
            ...
            <Link href="/pricing" className="block w-full text-center py-2.5 border-2 border-brand-blue text-brand-blue font-semibold rounded-[10px] text-sm hover:bg-brand-blue hover:text-white transition-colors">Get Started</Link>
          ...
          <div className="bg-white rounded-[16px] p-8 border-2 border-brand-blue scale-105 shadow-card relative">
            ...
            <Link href="/pricing" className="block w-full text-center py-2.5 bg-brand-blue text-white font-semibold rounded-[10px] text-sm hover:bg-brand-dark transition-colors">Start 7-Day Free Trial</Link>
            <ul className="mt-6 space-y-3 text-sm text-brand-dark">
          ...
          <div className="bg-white rounded-[16px] p-8 border-2 border-amber-400 relative shadow-lg">
            ...
            <p className="text-xs text-brand-muted mb-6">7-day free trial, cancel anytime</p>
            <Link href="/pricing" className="block w-full text-center py-2.5 bg-amber-500 text-white font-semibold rounded-[10px] text-sm hover:bg-amber-600 transition-colors">Start 7-Day Free Trial</Link>
            <ul className="mt-6 space-y-3 text-sm text-brand-dark">
  ...
  return (
    <section className="py-20 md:py-28 bg-brand-blue text-white text-center">
      <div className="max-w-2xl mx-auto px-6">
        ...
        <p className="text-white/80 mb-8 leading-relaxed">Join dental offices worldwide using ReviewFlow. Start free, no credit card required.</p>
    ...
    <footer className="bg-brand-dark text-white py-12">
      <div className="max-w-6xl mx-auto px-6">
        <div className="flex flex-col md:flex-row items-center justify-between gap-6">
          ...
          <div className="text-sm text-white/60">© 2026 ReviewFlow. All rights reserved.</div>
...
export default function HomePage() {
  return (
    <main className="min-h-screen">
      ...
      <div className="pt-20">
        <HeroSection />
        <div id="features"><HowItWorks /><FeaturesSection /></div>
        <TestimonialsSection />
